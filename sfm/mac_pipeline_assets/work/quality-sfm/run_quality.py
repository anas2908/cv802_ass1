...
name=sys.argv[1]
if not name.endswith("_quality"):
    raise ValueError("Use a separate *_quality dataset; baseline results are preserved.")
...
report["mode"]="fixed_pose_sparse_refinement"
report["note"]="Camera poses are inherited from the baseline; registered image count is not new evidence of improved camera estimation."
report["refinement_recipe"]=json.loads((dataset/"sfm_refine.json").read_text())
