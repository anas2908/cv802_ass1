#!/usr/bin/env python3
"""Filter existing body/crutch SfM points using masks and reviewed crutch regions.
...
_helper_path = HERE / "crutch_protection/build_protection.py"
_spec = importlib.util.spec_from_file_location("crutch_regions", _helper_path)
crutch_regions = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(crutch_regions)


def combine_selections(body, protected, distinct_views, min_track_views=3):
    return (np.asarray(body, dtype=bool) | np.asarray(protected, dtype=bool)) & (np.asarray(distinct_views) >= min_track_views)


def validate_protection(protection):
    selection = protection.get("selection", {})
    numeric_keys = ("minimum_annotated_views", "minimum_distinct_track_views",
        "minimum_pairwise_camera_ray_separation_degrees_for_confirming_triple",
        "max_mean_baseline_pixel_error", "minimum_triangulation_angle_degrees")
    if not all(isinstance(selection.get(key), (int, float)) and np.isfinite(selection[key]) for key in numeric_keys):
        raise ValueError("Protection thresholds must be finite numbers")
    if (protection.get("ready_for_filter") is not True
            or protection.get("review", {}).get("status") != "approved_for_experiment"
            or selection.get("minimum_annotated_views", 0) != 3
            or selection.get("minimum_distinct_track_views", 0) < 3
            or selection.get("minimum_pairwise_camera_ray_separation_degrees_for_confirming_triple", 0) < 15
            or selection.get("max_mean_baseline_pixel_error", float("inf")) > 3
            or selection.get("minimum_triangulation_angle_degrees", 0) < 1.5):
        raise ValueError("Protection must have reviewed regions, three separated visible confirmations and quality gates")
    if len(protection.get("annotated_image_corridors", {})) < 3 or set(protection.get("regions", {})) != {"A", "B"}:
        raise ValueError("Protection requires both crutches and at least three annotated views")
    for region in protection["regions"].values():
        for capsule in region["capsules"]:
            start, end = np.asarray(capsule["start_world"]), np.asarray(capsule["end_world"])
            radius = float(capsule["radius"])
            if (start.shape != (3,) or end.shape != (3,) or not np.isfinite([*start, *end, radius]).all()
                    or radius <= 0 or np.linalg.norm(end - start) <= 1e-12):
                raise ValueError("Invalid finite bounded capsule")


def evaluate_protection(model, xyz, protection):
    validate_protection(protection)
    for filename, digest in protection["source_model_sha256"].items():
        if file_hash(Path(filename)) != digest:
            raise ValueError("Reviewed baseline source model changed")
    reference = pycolmap.Reconstruction(protection["source_model"])
    baseline_images = {im.name: im for im in reference.images.values()}
    candidate_images = {im.name: im for im in model.images.values()}
    if set(baseline_images) != set(candidate_images):
        raise ValueError("Reviewed baseline and candidate must have identical registered image names")
    pose_delta, calibration_delta = 0., 0.
    for name, before in baseline_images.items():
        after = candidate_images[name]
        pose_delta = max(pose_delta, float(np.max(np.abs(before.cam_from_world().matrix() - after.cam_from_world().matrix()))))
        original = reference.cameras[before.camera_id]
        actual = model.cameras[after.camera_id]
        expected = pycolmap.Camera(original.todict())
        expected.rescale(actual.width, actual.height)
        if expected.model != actual.model:
            raise ValueError("Candidate camera model differs from reviewed baseline")
        calibration_delta = max(calibration_delta, float(np.max(np.abs(expected.params - actual.params))))
    if pose_delta > 1e-10 or calibration_delta > 1e-8:
        raise ValueError("Candidate does not preserve the reviewed world frame and scaled calibration")
    selection = protection["selection"]
    support, separated, per_view = crutch_regions.confirmation_gate(model, xyz,
        protection["annotated_image_corridors"],
        selection["minimum_pairwise_camera_ray_separation_degrees_for_confirming_triple"])
    measures = point_measurements(model, image_states(reference))
    if not np.array_equal(measures["xyz"], xyz):
        raise AssertionError("Crutch measurements changed point ordering")
    quality = ((measures["distinct_views"] >= selection["minimum_distinct_track_views"])
        & (measures["normalized"] <= selection["max_mean_baseline_pixel_error"])
        & (measures["angles"] >= selection["minimum_triangulation_angle_degrees"])
        & measures["all_observations_valid"])
    selected = {label: crutch_regions.capsule_gate(xyz, region["capsules"]) & separated & quality
        for label, region in protection["regions"].items()}
    protected = np.logical_or.reduce(list(selected.values()))
    stats = {"reference_model": protection["source_model"],
        "max_pose_element_difference": pose_delta,
        "max_scaled_calibration_parameter_difference": calibration_delta,
        "candidate_protected_points": int(protected.sum()),
        "crutch_points": {label: int(mask.sum()) for label, mask in selected.items()},
        "confirmed_view_counts_retained_min_max": [int(support[protected].min()), int(support[protected].max())] if protected.any() else [],
        "per_view": per_view,
        "point_id_policy": "Recomputed on candidate coordinates; baseline point IDs are never transferred"}
    return protected, selected, support, separated, stats
    ...
    for row, (a, b, view) in enumerate(((0, 2, "Front"), (1, 2, "Side"), (0, 1, "Top"))):
        for col, selected in enumerate((np.ones(len(xyz), dtype=bool), keep)):
            ...
            ax.set(xlim=(center[a] - radius, center[a] + radius),
                   ...
                   title=f"{view} — {'Source' if col == 0 else 'Body + crutch cleanup'}: {selected.sum():,} points")
            ax.set_aspect("equal", adjustable="box")
            ax.grid(alpha=.15)
    fig.suptitle(f"Body masks + reviewed crutch protection — {label}\n{consensus:.0%} body foreground agreement OR verified crutch points; identical view and scale", fontsize=13)
...
def arguments():
    ...
    parser.add_argument("--source", type=Path, default=HERE.parents[1] / "black_shirt_crutches_quality/subject_preview")
    parser.add_argument("--manifest", type=Path, default=HERE / "body_mask_manifest.json")
    parser.add_argument("--protection", type=Path, default=HERE / "crutch_protection/protection_approved.json")
    parser.add_argument("--candidate-review", type=Path, default=HERE / "crutch_protection/quality_candidate_review")
    parser.add_argument("--qc-only", action="store_true", help="Write candidate point/photo overlays for review; do not filter")
    parser.add_argument("--output-parent", type=Path, default=HERE)
    parser.add_argument("--variants", nargs="+", default=["body_mask_consensus_90_crutches:0.90", "body_mask_consensus_97_crutches:0.97"],
...
def main():
    ...
    if not manifest.get("complete") or not manifest.get("ready_for_body_filter"):
        raise ValueError("Body mask manifest must be complete and ready_for_body_filter after review")
    ...
    watched = {p: file_hash(p) for p in [manifest_path, protection_path, _helper_path,
        ROOT / "work/quality-sfm/compare_models.py", analysis_path, metadata_path, source_database,
        *(p for p in model_path.iterdir() if p.is_file())]}
    watched.update({Path(filename): digest for filename, digest in protection["source_model_sha256"].items()})
    ...
    protected, crutch_selections, annotated_support, separated, protection_stats = evaluate_protection(model, xyz, protection)
    if not protected.any():
        raise ValueError("No candidate crutch points passed the reviewed protection recipe")
    review_path = args.candidate_review.resolve()
    review_signature = {"source_model_sha256": {str(p): watched[p] for p in model_path.iterdir() if p.is_file()},
        "protection_sha256": watched[protection_path], "helper_sha256": watched[_helper_path]}
    if args.qc_only:
        if review_path.exists():
            raise FileExistsError(f"Preserving existing candidate review: {review_path}")
        review_path.mkdir(parents=True)
        crutch_regions.SOURCE = source
        crutch_regions.draw_overlays(review_path / "projection_qc.png", model, xyz, crutch_selections,
            protection["annotated_image_corridors"], protection["regions"])
        np.savez_compressed(review_path / "candidate_existing_points.npz", source_point_ids=ids,
            xyz=xyz, protected=protected, crutch_A=crutch_selections["A"], crutch_B=crutch_selections["B"],
            annotated_view_support=annotated_support, three_separated_confirming_views=separated)
        report = {"ready_for_filter": False, "status": "candidate_projection_review_required",
            "source_dataset": str(source), "source_points": len(ids), **review_signature, **protection_stats}
        (review_path / "review.json").write_text(json.dumps(report, indent=2) + "\n")
        for path, digest in watched.items():
            if file_hash(path) != digest:
                raise RuntimeError(f"Read-only candidate QC input changed: {path}")
        print(json.dumps(report, indent=2))
        return
    review_file = review_path / "review.json"
    review = json.loads(review_file.read_text())
    if review.get("ready_for_filter") is not True or review.get("status") != "approved_for_experiment":
        raise ValueError("Candidate crutch projections must pass review before filtering")
    if any(review.get(key) != value for key, value in review_signature.items()):
        raise ValueError("Candidate source or protection changed after its projection review")
    if review.get("candidate_protected_points") != int(protected.sum()):
        raise AssertionError("Candidate protected point count changed since its review")
    watched[review_file] = file_hash(review_file)
    watched[review_path / "projection_qc.png"] = file_hash(review_path / "projection_qc.png")
    ...
    body_masks = {name: choose_points(foreground_counts, in_frame_counts, distinct_views, reliable,
        fraction, args.min_positive_view_fraction, args.min_track_views) for name, fraction in args.variants}
    masks = {name: combine_selections(body, protected, distinct_views, args.min_track_views)
        for name, body in body_masks.items()}
    ...
    for name, fraction in args.variants:
        ...
        with tempfile.TemporaryDirectory(prefix=f".{name}_", dir=output_parent) as temporary:
            ...
            (staging / "body_mask_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
            shutil.copy2(protection_path, staging / "crutch_protection.json")
            shutil.copy2(review_file, staging / "candidate_projection_review.json")
            shutil.copy2(review_path / "projection_qc.png", staging / "crutch_projection_qc.png")
            np.savez_compressed(staging / "mask_votes.npz", source_point_ids=ids,
                ...
                distinct_track_views=distinct_views, retained=selected, body_retained=body_masks[name],
                crutch_protected=protected, crutch_A=crutch_selections["A"], crutch_B=crutch_selections["B"],
                annotated_crutch_support=annotated_support, three_separated_confirming_views=separated)
            stats = {"experiment": name, "source_dataset": str(source), "source_model": str(model_path),
                ...
                "body_mask_retained_points": int(body_masks[name].sum()),
                "crutch_protected_points": int(protected.sum()),
                "crutch_only_added_to_body_selection": int((protected & ~body_masks[name]).sum()),
                "crutch_protection": protection_stats,
                ...
                "filter": {"selection_rule": "Body-mask consensus OR reviewed capsule plus three separated visible-crutch corridor confirmations; distinct-track-view gate applies to both",
                    "foreground_agreement": fraction,
                ...
                "limitations": [*protection["limitations"], "Masks can remove valid body details or retain occluded background. Subject motion and segmentation mistakes can affect votes.",
            ...
            (staging / "README.md").write_text(
                f"# Body masks with reviewed crutch protection: {name}\n\n"
                f"Retained {selected.sum():,} of {len(ids):,} existing sparse points. The body mask requires {fraction:.0%} foreground agreement "
                f"and at least {math.ceil(args.min_positive_view_fraction * reliable)} positive views. A point also survives if it lies within a reviewed "
                "crutch region, projects into visible crutch corridors in at least three pairwise separated views (15 degrees), and passes error/angle checks. "
                f"Every retained point has at least {args.min_track_views} distinct track images.\n\n"
                f"Body masks retain {body_masks[name].sum():,} points; reviewed crutch protection adds {(protected & ~body_masks[name]).sum():,} more. "
                f"Total protected crutch-region points: {protected.sum():,}. Baseline point IDs were never transferred; protection was evaluated anew on this quality source.\n\n"
                ...
                "the body mask manifest, reviewed crutch parameters, candidate projection review and `provenance.json` record the selection and its inputs.\n\n"
                "No artificial points or dense geometry were added. Segmentation errors or subject motion can remove genuine body points; "
                "foreground agreement does not guarantee that every retained point belongs to the person. Crutch regions can retain nearby hand/body points "
                "and ambiguous floor-contact points; missing metal shaft geometry cannot be restored by this filter.\n")
