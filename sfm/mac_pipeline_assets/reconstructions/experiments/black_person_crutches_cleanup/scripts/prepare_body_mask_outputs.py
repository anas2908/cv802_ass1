...
SFM=ROOT.parents[2]
INPUT=json.loads((ROOT/'body_input_manifest.json').read_text())
BOXES=json.loads((SFM/'work/quality-sfm/photos_native/black_shirt_crutches/boxes.json').read_text())
records={}
for row in INPUT:
...
manifest={'schema_version':1,'source_dataset':str((SFM/'reconstructions/black_shirt_crutches_quality').resolve()),'complete':complete,'ready_for_filter':False,'ready_for_body_filter':complete and counts['valid']>=3,'intended_use':'Body-only masks: require explicit independently verified crutch protection for any final cleanup.','crutch_coverage_guaranteed':False,
...
(ROOT/'body_mask_manifest.json').write_text(json.dumps(manifest,indent=2))
...
priority += [name for name,box in BOXES.items() if box.get('status')=='not_detected']
...
for start in range(0,len(chosen),12):
    ...
    sheet.save(ROOT/f'body_mask_qc_{start:03d}.jpg',quality=94)
